...
mainTab:AddLabel("COPIED SCRIPT")
...
-- Create the Rebirth Farm folder
local rebirthFarmFolder = miscTab:AddFolder("Fuck Farm")
...
local function rebuildTargetDropdown()
    local choices = {}
    local map     = {} -- displayName  -> Player instance
    ...
    -- if the dropdown already exists just update its choices
    if killerTab.targetDropdown then
        killerTab.targetDropdown:Clear()
        for _, opt in ipairs(choices) do killerTab.targetDropdown:Add(opt) end
    else
        -- first time creation
        killerTab.targetDropdown = killerTab:AddDropdown("TargetPlayerDrop", function(text)
            local plr = map[text]
            if plr then
                _G.targetPlayer = plr.Name .. " (" .. plr.DisplayName .. ")"
...
-- keep it in sync
game.Players.PlayerAdded:Connect(rebuildTargetDropdown)
game.Players.PlayerRemoving:Connect(rebuildTargetDropdown)

killerTab:AddButton("🔄 Refresh List", function()
    rebuildTargetDropdown()          -- re-scan every player
    game:GetService("StarterGui"):SetCore("SendNotification",{
        Title = "Player list refreshed",
        Text  = "Dropdown updated with current server players.",
        Duration = 2
    })
end)

-- Improved auto-kill target feature
local autoKillTargetSwitch = killerTab:AddSwitch("Kill Target Automatically", function(bool)
...
-- ===== VIEW / UN-VIEW PLAYER =================================================
local viewBtn, unviewBtn -- references so we can enable/disable them

-- helper to spectate the currently-selected target
local function spectateTarget()
    if not _G.targetPlayer or _G.targetPlayer == "" then return end
    local name = _G.targetPlayer:match("^([%w_]+)")          -- strip " (DisplayName)"
    local tgt  = game.Players:FindFirstChild(name)
    if tgt and tgt.Character and tgt.Character:FindFirstChildOfClass("Humanoid") then
        workspace.CurrentCamera.CameraSubject = tgt.Character:FindFirstChildOfClass("Humanoid")
    end
end

-- helper to return camera to yourself
...
viewBtn   = killerTab:AddButton("View Target", spectateTarget)
unviewBtn = killerTab:AddButton("Un-view / Stop Spectating", unspectate)
...
local killingTab = window:AddTab("KillerV2")
...
killingTab:AddSwitch("Fast Punch", function(state)
...
local function startViewAndKill()
    if not _G.targetPlayer or _G.targetPlayer == "" then return end

    viewing = true

    viewLoop = spawn(function()
        while viewing do
            local name = _G.targetPlayer:match("^([%w_]+)")
            local tgt = game.Players:FindFirstChild(name)

            -- Wait until target is in-game and has a character
            while viewing and (not tgt or not tgt.Character or not tgt.Character:FindFirstChildOfClass("Humanoid")) do
                task.wait(0.5)
                tgt = game.Players:FindFirstChild(name)
            end
            if not viewing then break end

            -- Lock camera
            workspace.CurrentCamera.CameraSubject = tgt.Character:FindFirstChildOfClass("Humanoid")

            -- Kill loop while target is alive
            while viewing and tgt and tgt.Character and tgt.Character:FindFirstChild("HumanoidRootPart") and tgt.Character.Humanoid.Health > 0 do
                pcall(function()
                    killPlayer(tgt)
                end)
                task.wait(0.1)
            ...
            -- Target died or left; wait for respawn
            if viewing then
                unspectate()
                task.wait(1) -- brief pause before restart
            end
        end
...
killingTab:AddButton("View & Kill Target (Auto-Restart)", function()
    if not viewing then
        startViewAndKill()
    end
end)

killingTab:AddButton("Stop Viewing & Killing", function()
    viewing = false
    if viewLoop then
        -- no way to truly kill a spawn, so we just flag it
        viewLoop = nil
    end
    unspectate()
end)
...
killingTab:AddDropdown("Select Kill Method", {"Teleport", "Non-Teleport"}, function(method)
    getgenv().killMethod = method
    print("Kill Method set to: " .. method)
end)
killingTab:AddDropdown("Whitelist Players", function()
...
killingTab:AddTextBox("Player Username", function(name)
    getgenv().targetPlayerName = name
end)
killingTab:AddSwitch("Auto Fast Kill Player", function(state)
...
a.Ambient = Color3.fromRGB(255, 225, 235)      -- petal-pink fill
a.Brightness = 1.1                             -- airy daylight
a.ColorShift_Bottom = Color3.fromRGB(255, 210, 220)
a.ColorShift_Top = Color3.fromRGB(255, 240, 245) -- almost-white sky
a.EnvironmentDiffuseScale = 0.3                -- soft diffuse
a.EnvironmentSpecularScale = 0.4               -- mild highlights
a.GlobalShadows = true
a.OutdoorAmbient = Color3.fromRGB(255, 220, 230)
a.ShadowSoftness = 0.35                        -- blossom-soft shadows
a.GeographicLatitude = 20                      -- low spring sun
a.ExposureCompensation = 0.15                  -- light, dreamy over-exposure

local b = Instance.new("BloomEffect", a)
b.Enabled = true
b.Intensity = 0.22                             -- delicate glow
b.Size = 2600
b.Threshold = 0.65

local c = Instance.new("ColorCorrectionEffect", a)
c.Brightness = 0.1
c.Contrast = 0.08                              -- very gentle
c.Enabled = true
c.Saturation = 0.3                             -- pastel pop
c.TintColor = Color3.fromRGB(255, 182, 193)    -- sakura-pink tint
